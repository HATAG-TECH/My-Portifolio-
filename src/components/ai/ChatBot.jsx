  ...
  return (
    ...
    >
      <div className={`max-w-[88%] rounded-2xl px-3 py-2 text-xs leading-relaxed shadow-md ${bubbleClass}`}>
        ...
        <span className="mt-1 hidden text-[10px] opacity-70 group-hover:block">
    ...
    <>
      <div className="fixed bottom-5 right-5 z-40">
        <button
          type="button"
          onClick={toggleOpen}
          className={`group relative flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-tr text-xl text-white shadow-lg transition duration-300 hover:scale-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ${floatingButtonClass}`}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.section
            ...
            className={`fixed z-40 backdrop-blur-xl ${isDesktop ? 'bottom-20 right-5 w-[350px]' : 'inset-0 w-full'} ${basePanelClass} ${isDesktop ? 'rounded-2xl' : 'rounded-none'} border shadow-2xl`}
            style={panelStyle}
          >
            <header
